  ...
  return (
    <>
      <div className="flex-col">
        ...
        <div className="overflow-auto scrollbar-hide h-[25rem]">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <Link to={`/otherProfile/${obj._id}`}>
                      <div>
                        ...
                        {obj.workingOn && obj.workingOn.length > 0 ? (
                          <p className="font-semibold text-sm">
                            skilled in {obj.workingOn.join(" , ")}
                          </p>
                        ) : (
                          <p>{""}</p>
                        )}
          ...
          {!showFollowers && !showFollowing && (
            <>
              {allUsers.length === 0 ? (
                <p className="text-center p-2 font-semibold">No users found.</p>
              ) : (
                allUsers.map((user) => (
                  <Card key={user._id}>
                    <div className="flex gap-4 justify-between">
                      <div className="flex items-center gap-3">
                        <div className="border rounded-full">
                          <img
                            className="w-14 h-14 border border-green-400 rounded-full"
                            src={user.imageName}
                            alt=""
                          />
                        </div>
                        <Link to={`/otherProfile/${user._id}`}>
                          <div>
                            ...
                            {user.workingOn && user.workingOn.length > 0 ? (
                              <p className="font-semibold text-sm">
                                skilled in {user.workingOn.join(",")}
                              </p>
                            ) : (
                              <p>{""}</p>
                            )}
